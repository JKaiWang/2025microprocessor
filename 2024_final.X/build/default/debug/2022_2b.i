...
volatile int key = 0;
volatile int seq[] = {1, 0 , -1, 0};
volatile int i =0 ;
volatile int total_cycle = 0;
volatile int divide = 0 ;
...
# 407 "2022_2b.c"
    ...
    while (1) {
        ...
        if(cycle >0 && change_state == 1 && key == 1){


            angle = divide*(total_cycle-cycle)*seq[i];
            printf("cycle: %ddivide: %d\n" , cycle , angle);
            if(seq[i] == 1|| seq[i] == -1){
