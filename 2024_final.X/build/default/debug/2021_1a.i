...
volatile int mode = 0;
volatile int n_steps = 0;
volatile int step_cnt = 0;
volatile int direction = 1;
volatile int auto_run = 0;
void button_pressed() {
    if (mode == 1) {

        if (step_cnt < n_steps) {

            set_servo_angle(get_servo_angle() + (180 / n_steps));
        }
        else if (step_cnt < 2 * n_steps) {
...
void keyboard_input(char *str) {
    ...
    if (mode == 1 || mode == 2) {
        ...
        if (x >= 1 && x <= 9) {
            n_steps = x;
            printf("n = %d, need %d steps\r", n_steps, 2 * n_steps);
            set_servo_angle(-90);
        }
    }
}
