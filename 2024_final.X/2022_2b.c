...
void Init_Timer1_4MHz_1ms(void)
{
    ...
    // Clock = Fosc/4 = 1MHz → 1 tick = 1us
    T1CONbits.TMR1CS = 0;   // Use internal clock (Fosc/4)
    T1CONbits.T1CKPS = 0b00; // Prescaler = 1:1
    T1CONbits.T1OSCEN = 0;   // Disable LP oscillator (not needed)

    // Preload = 65536 - 1000 = 64536
    ...
    // IPR1bits.TMR1IP = 0; // if low priority
    // Turn on Timer1
    T1CONbits.TMR1ON = 1;
}
