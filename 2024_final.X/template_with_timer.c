...
void Timer1_Init(void)
{
    ...
    T1CONbits.TMR1ON = 0;     //Normally off
}
void Timer2_Init(unsigned char prescale, unsigned char postscale, unsigned char pr2_value)
{
    T2CONbits.TMR2ON = 0;        // Stop Timer2
    T2CONbits.T2CKPS = prescale; // Prescaler (0=1, 1=4, 2=16)
    T2CONbits.TOUTPS = postscale; // Postscaler 1~16  (0=1:1)
    PR2 = pr2_value;             // Period value
...
void Timer3_Init(void)
{
    ...
    T3CONbits.TMR3ON = 0;     //  Normally off
}
void Timer1_delay_1ms(void)
{
    T1CONbits.TMR1ON = 0;

    TMR1H = 0xFC;    // preload for 1ms (4MHz)
    TMR1L = 0x18;

    PIR1bits.TMR1IF = 0;
    T1CONbits.TMR1ON = 1;

    while(!PIR1bits.TMR1IF);  // wait overflow
...
// --------------- TODO ------------------
volatile int mode = 0;
volatile int n_steps = 0;     // n (user input)
volatile int step_cnt = 0;    // current step count
volatile int direction = 1;   // +1 going upward, -1 going downward
volatile int auto_run = 0;    // mode2 是否開始自動跑 
void button_pressed() {
    // if (mode == 1) {

    //     if (step_cnt < n_steps) {
    //         // going upward: -90 → 0 → 90
    //         set_servo_angle(get_servo_angle() + (180 / n_steps));
    //     }
    //     else if (step_cnt < 2 * n_steps) {
    //         // going downward: 90 → 0 → -90
    //         set_servo_angle(get_servo_angle() - (180 / n_steps));
    //     }
